...
function ESPLib.new(groupBehaviour)
    ...
    self._components = {
        ...
        name = true,
        distance = true,
        flags = groupBehaviour.Flags ~= nil,
        snapline = false
    }
    self._running = true
...
function ESPLib:_BoundingBox(instance)
    ...
    for _, child in ipairs(children) do
        ...
        if allVisible and self._IsBasePart(child) then
            ...
            local childSize = self._gb_measureEntry and self._gb_measureEntry(instance) or
                                  (DEFAULT_PARTS_SIZING[childName] ~= nil and DEFAULT_PARTS_SIZING[childName] or
                                      Vector3(1, 1, 1))
            ...
            local vertices = math.abs(vec3Magnitude(childSize)) < 0.2 and {childOrigin} or
                                 {childOrigin + rotateY(Vector3(-halvedSize.x, 0, -halvedSize.z), angle) +
                    Vector3(0, halvedSize.y, 0),
                                  childOrigin + rotateY(Vector3(halvedSize.x, 0, halvedSize.z), angle) +
                    Vector3(0, -halvedSize.y, 0)}
...
function ESPLib:Add(entry)
    ...
    self._objects[entry] = {
        ...
        ['_drawings'] = {espBbox, espBboxOutlineInner, espBboxOutlineOuter, espName, espDistance, espSnapline,
                         unpack(espFlags)}
    }
...
function ESPLib:Step()
    -- refresh container
    local now = os.clock()
    if self._objectContainer and now - self._containerLastUpdate >
        (self._gb_mode and (self._gb_mode == 1 and 0 or 0.33) or 1) then
...
-- Fixed ValidateEntry function
local espGroup = ESPLib.new {
    ValidateEntry = function(entry)
        -- Add safety checks
        if not entry or not entry:FindFirstChild("Progress") then
            return false
        end

        -- Safe character name check
        local characterName = Player.Character and Player.Character.Name or ""

        return entry.Name == "Generator" and entry.Progress.Value ~= 100 and not killers[characterName]
    end
}
...
local espGroup = ESPLib.new {
    ValidateEntry = function(entry)
        -- Check if entry exists and has required components
        if not entry or entry.Name ~= "Generator" then
            return false
        end

        local progress = entry:FindFirstChild("Progress")
        if not progress or progress.Value == 100 then
            return false
        end

        -- Only check killer status if character exists
        if Player.Character and Player.Character.Name then
            return not killers[Player.Character.Name]
        end

        -- If no character, still show generators (safer default)
        return true
    end,

    FetchEntryName = function(entry)
        return string.format("Generator: %d%%", entry.Progress.Value)
    end
}

espGroup:SetGroupContainer(workspace.Map.Ingame.Map)

pcall(function()
    while true do
        -- \\ change it when game restarts
